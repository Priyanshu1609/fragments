...
const SelectNFT: React.FC<SelectProposalProps> = ({
  setCurrentStep,
  handleBack
}) => {
  ...
  const onSubmitHandler: React.MouseEventHandler<HTMLButtonElement> = (e) => {
    ...
    setCurrentStep(ProposalStep.ListNFT)
  }
  ...
  const getNFTs = async () => {
    ...
    try {
      ...
      fetch(`https://testnets-api.opensea.io/api/v1/assets?owner=${vault}&order_direction=desc&limit=20&include_orders=false`, options)
  ...
  return (
    <div className='pb-8 text-lg'>
      ...
      <div className='flex justify-evenly mt-8'>
        <button onClick={handleBack} className='w-44 px-3 py-2 rounded-lg  bg-[#232529]  text-white flex items-center space-x-4'>
